...
public class SnowballBowDiamondItem extends Item {
	public SnowballBowDiamondItem() { super(new Item.Properties().durability(1561)); }
	...
	@Override public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entityLiving, int timeLeft) { fire(world, entityLiving, itemstack, 12.5D, 10, 1.35F); }
	...
	private void fire(Level world, LivingEntity entityLiving, ItemStack itemstack, double damage, int knockback, float velocity) {
		if (!world.isClientSide && entityLiving instanceof ServerPlayer entity) {
			...
			if (entity.getAbilities().instabuild || stack != ItemStack.EMPTY) {
				SnowballProjectileEntity projectile = SnowballProjectileEntity.shoot(AngryBowEntities.SNOWBALL_BOW_DIAMOND.get(), world, entity, world.random, false, damage, knockback, velocity);
